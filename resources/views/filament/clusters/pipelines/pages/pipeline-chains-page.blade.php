<x-filament-panels::page>
  ...
  @if($base_entity_id)
    @php
      $report = app(\Nicole\Box\Core\Services\Calculator\PipelineTreeService::class)->analyzeTree($base_entity_id, $pipeline_code, $base_entity_type ?? 'product_variant');
      $rootVariant = \Nicole\Box\Core\Models\ProductVariant::find($base_entity_id);
      $isRootActive = $rootVariant?->product?->is_active ?? false;
    @endphp

    <div class="mb-6">
      <div
        class="flex items-center justify-between mb-4 bg-white dark:bg-gray-900 p-4 rounded-xl border border-gray-200 dark:border-gray-800">
        <div>
          <span
            class="text-xs text-gray-500 uppercase tracking-wider font-semibold">{{ __('Configuring Chain') }}</span>
          <h2 class="text-lg font-bold text-gray-900 dark:text-white">
            {{ $report['name'] ?? ($rootVariant?->product?->name ?? '') }}
          </h2>
        </div>
      ...
      @if($report)
        @include('nicole-core::filament.clusters.pipelines.components.tree-status-panel', [
            ...
            'entityType' => $base_entity_type ?? 'product_variant',
