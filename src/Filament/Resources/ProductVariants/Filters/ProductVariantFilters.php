...
namespace Nicole\Box\Core\Filament\Resources\ProductVariants\Filters;
    ...
    return [
      ...
      Filter::make('category_id')
        ...
        ->query(function (Builder $query, array $data): Builder {
          ...
          return $query->whereHas('product', function (Builder $q) use ($allTargetIds) {
            $q->whereIn('category_id', $allTargetIds);
          });
        }),
      ...
      Filter::make('advanced_filters')
        ...
        ->indicateUsing(function (array $data): array {
          ...
          if (filled($data['stock_from']) || filled($data['stock_to'])) {
            $from = $data['stock_from'] ?? '0';
            $to = $data['stock_to'] ?? '∞';
            $indicators[] = Indicator::make("Остаток: {$from} - {$to}")
              ->removeField('stock_from')
              ->removeField('stock_to');
          }
        ...
        ->query(function (Builder $query, array $data): Builder {
          return $query
            ->when(
              ...
              fn($q) => $q->whereHas('product', fn($pQ) => $pQ->whereIn('product_type_id', (array)$data['product_type_id']))
            )
            ->when(
